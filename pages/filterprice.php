...
$harga = isset($_GET['harga']) ? $_GET['harga'] : 'asc'; 
...
if (!in_array($harga, ['asc', 'desc'])) {
    $harga = 'asc';
}

$produk = query("SELECT * FROM tb_barang WHERE kategori = '$kategori' ORDER BY harga $harga LIMIT $limit OFFSET $offset");
    ...
    <nav aria-label="Page navigation example">
        <ul class="pagination justify-content-center">
            <li class="page-item <?= ($page <= 1) ? 'disabled' : '' ?>">
                <a class="page-link" href="?kategori=<?= $kategori ?>&harga=<?= $harga ?>&page=<?= $page - 1 ?>" tabindex="-1">Previous</a>
            </li>
            <?php for ($i = 1; $i <= $total_pages; $i++) { ?>
                <li class="page-item <?= ($page == $i) ? 'active' : '' ?>"><a class="page-link" href="?kategori=<?= $kategori ?>&harga=<?= $harga ?>&page=<?= $i ?>"><?= $i ?></a></li>
            <?php } ?>
            <li class="page-item <?= ($page >= $total_pages) ? 'disabled' : '' ?>">
                <a class="page-link" href="?kategori=<?= $kategori ?>&harga=<?= $harga ?>&page=<?= $page + 1 ?>">Next</a>
            </li>
        </ul>
    </nav>
  </div>
